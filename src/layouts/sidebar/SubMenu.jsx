      ...
      >
        <data.icon
          ...
          className={`min-w-max group-hover/navitemsub:text-menuItemColor ${
            ...
            pathname.includes(data.name) || pathname.includes(data.prefix) ? "text-white" : "text-menuItemIcon"
          }`}
        />
        ...
        <p
          className={`flex-1 capitalize  ${
            pathname.includes(data.name)
              ? "!text-white"
              : pathname.includes(data?.prefix) && "!text-white"
          } `}
        >
          {data.name}
        </p>
            ...
            >
              {menu?.withSubMenu3 ? (
                <div
                  ...
                  className="link-sub-menu3   capitalize relative hover:text-white hover:cursor-pointer hover:no-underline  visited:no-underline active:no-underline"
                >
                    ...
                    <div className="flex items-center gap-x-1">
                      <menu.icon
                        ...
                        className={`min-w-max group-hover/navitemsub:text-menuItemColor ${
                          ...
                          pathname.includes(menu.name) || pathname.includes(menu.prefix) ? "text-white" : "text-menuItemIcon"
                        }`}
                      />
                      ...
                      {menu?.menus?.map((menu3) => (
                        ...
                        >
                          <NavLink
                            to={
                              // routeMerge
                                // ? `/${menu.name}${menu3.route}`
                                // : 
                                `${menu3.route}`
                            }
                            // link-sub-menu3
                            className="link-sub-menu4  capitalize relative hover:text-white hover:cursor-pointer hover:no-underline  visited:no-underline active:no-underline"
                          >
            ...
            
                  {
                    menu?.name?.toLowerCase() === 'message room'   ? (
                      ...
                      className={` ${menu?.icon ? 'link-sub-menu3' : 'link-sub-menu'}    capitalize relative hover:text-white hover:cursor-pointer hover:no-underline  visited:no-underline active:no-underline`}
                    >
                      {pathname.includes(menu.route) && (
                        <span className={`w-2 h-2 rounded-full bg-btnColor absolute left-[0.1.7rem] duration-200 top-3 transition-all ${menu?.icon ? 'left-[0rem]' : 'left-[0.30rem]'}`}></span>
                      )}
                              <span>
                                {menu.name}
                              </span>
                        ...
                        <NavLink
                          to={
                            routeMerge ? `/${data.name}${menu.route}` : `${menu.route}`
                          }
                          className={` ${menu?.icon ? 'link-sub-menu3' : 'link-sub-menu'}    capitalize relative hover:text-white hover:cursor-pointer hover:no-underline  visited:no-underline active:no-underline`}
                        >
                          {pathname.includes(menu.route) && (
                                  ...
                                  <div className="flex items-center gap-x-1 ">
                                    <menu.icon
                                      ...
                                      className={`min-w-max group-hover/navitemsub:text-menuItemColor ${
                                        ...
                                        pathname.includes(menu.name) ? "text-white" : "text-menuItemIcon"
                                      }`}
                                    />
