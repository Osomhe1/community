...
const HistoryTable = ({ view, setCurrentViewApproval,tableData }) => {
  ...
  const viewApproval = (data) => {
    setCurrentViewApproval(data);
    view();
  };
  ...
  const renderCell = React.useCallback((approval, columnKey) => {
    const cellValue = approval[columnKey];
    switch (columnKey) {
      case "name":
      ...
      case "responded_on":
        return (
        <div>
        {approval.status=='pending'?<p className="text-center">-</p>:<p>{cellValue}</p>}
        </div>
        );
      case "approved_by":
      return (
        <div>
        {approval.status!=='approved'?<p className="text-center">-</p>:<p>{cellValue}</p>}
        </div>
        );
      case "status":
        return (
       ...
       case "actions":
        return (
          ...
          {approval.status=='pending'?<Dropdown>
              <DropdownTrigger>
              ...
              <DropdownMenu>
                {/* <DropdownItem onClick={() => viewApproval(approval)}>View</DropdownItem> */}
                <DropdownItem>View</DropdownItem>
                <DropdownItem>Approve</DropdownItem>
                <DropdownItem>Reject</DropdownItem>
              </DropdownMenu>
            </Dropdown>:<Button size='sm' className='bg-[#00BCC2] font-medium text-white px-2' >View</Button>}
...
HistoryTable.propTypes = {
  setCurrentViewApproval: PropTypes.func,
  view: PropTypes.func,
