...
const columns = [
  ...
  {name: "staff Name", uid: "staff", sortable: true},
  {name: "request date", uid: "date", sortable: true},
  {name: "leave type", uid: "type", sortable: true},
  ...
  {name: "resumption Date", uid: "resumptionDate", sortable: true},
  {name: "leave Balance", uid: "leaveBalance", sortable: true},
  {name: "current Approval", uid: "currentApproval", sortable: true},
...
const LeaveReturnApprovalTable = ({handleOpenDrawer, rows}) => {
  ...
  const renderCell = React.useCallback((user, columnKey) => {
    ...
    switch (columnKey) {
      case "s_n":
        return (
          <p className="text-bold text-small">{user?._id}</p>
        );
      case "staff":
        return (
      ...
      case "resumptionDate":
        return <p className="text-bold text-small">{cellValue}</p>;
      case "leaveBalance":
        return <p className="text-bold text-center text-small">{cellValue}</p>;
     case "currentApproval":
        return <p className="text-bold text-small">{cellValue}</p>;
      case "actions":
        return <div className="relative flex gap-2">
        <Button className="bg-[#00bcc2] w-full text-white" onClick={()=>handleOpenDrawer(user)}>
              View
              </Button>
      </div>;
...
export default LeaveReturnApprovalTable;

LeaveReturnApprovalTable.propTypes = {
