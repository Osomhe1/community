...
const columns = [
  ...
  {name: "abbrevation", uid: "abbrevation", sortable: true},
...
const StaffRankTable = ({handleOpenDrawer, rows}) => {
  ...
  const filteredItems = React.useMemo(() => {
    ...
    if (hasSearchFilter) {
      filteredrows = filteredrows.filter((rank) =>
        rank.name.toLowerCase().includes(filterValue.toLowerCase())
      );
    }
    if (
      statusFilter !== "all" &&
      ...
      filteredrows = filteredrows.filter((rank) =>
        Array.from(statusFilter).includes(rank.status)
      );
    }
  ...
  const renderCell = React.useCallback((rank, columnKey) => {
    const cellValue = rank[columnKey];
    switch (columnKey) {
      case "name":
        return (
              ...
              <DropdownMenu>
                ...
                <DropdownItem >
                  <Button
                    ...
                    onClick={()=>handleOpenDrawer({rank, role: "edit"})}
                  >
                    Update
                  </Button>
                </DropdownItem>
