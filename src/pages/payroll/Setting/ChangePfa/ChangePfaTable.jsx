...
import { statusOptions } from "../Bank/data";
...
const columns = [
  ...
  {name: "previous Pfa", uid: "previousPfa", sortable: true},
  {name: "current Pfa", uid: "currentPfa", sortable: true},
...
const ChangePfaTable = ({handleOpenDrawer, rows}) => {
  ...
  const filteredItems = React.useMemo(() => {
    ...
    if (hasSearchFilter) {
      filteredrows = filteredrows.filter((pension) =>
        pension.name.toLowerCase().includes(filterValue.toLowerCase())
      );
    }
    if (
      statusFilter !== "all" &&
      ...
      filteredrows = filteredrows.filter((pension) =>
        Array.from(statusFilter).includes(pension.status)
      );
    }
  ...
  const renderCell = React.useCallback((pension, columnKey) => {
    const cellValue = pension[columnKey];
    switch (columnKey) {
      case "staffName":
        return (
      ...
      case "actions":
        return (
          <div className="relative flex justify-en items-center gap-2">
           <Dropdown>
              <DropdownTrigger aria-label="action_btn">
...
export default ChangePfaTable;

ChangePfaTable.propTypes = {
