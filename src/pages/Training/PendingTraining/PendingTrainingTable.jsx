  ...
  const filteredItems = React.useMemo(() => {
    ...
    if (hasSearchFilter) {
      filteredrows = filteredrows.filter((user) =>
        user.directorate.toLowerCase().includes(filterValue.toLowerCase())
      );
    }
    if (
      statusFilter !== "all" &&
      ...
      case "directorate":
        return (
          <div className="flex flex-col">
            <p className="text-bold text-tiny capitalize text-default-400 max-w-[35vh]">{cellValue}</p>
          </div>
        );
      case "title":
        return <p className="text-bold text-tiny capitalize text-default-400 max-w-[35vh]">{cellValue}</p>;
      case "desc":
        return <p className="text-bold text-small w-[35vh]">{cellValue}</p>;
