...
const columns = [
  ...
  {name: "nigeria Dta", uid: "nigeriaDta", sortable: true},
  {name: "nigeria Local Running %", uid: "nigeriaLocalRunning", sortable: true},
  {name: "nigeria Other Running %", uid: "nigeriaOtherRunning", sortable: true},
  {name: "oversees Dta", uid: "overseeDta", sortable: true},
  {name: "ovs Local Running %", uid: "ovsLocalRunning", sortable: true},
  {name: "ovs Other Running %", uid: "ovsOtherRunning", sortable: true},
...
const TrainingFeeTable = ({handleOpenDrawer, rows}) => {
  ...
  const filteredItems = React.useMemo(() => {
    ...
    if (hasSearchFilter) {
      filteredrows = filteredrows.filter((user) =>
        user.overseeDta.toLowerCase().includes(filterValue.toLowerCase())
      );
    }
    if (
      statusFilter !== "all" &&
  ...
  const renderCell = React.useCallback((user, columnKey) => {
    ...
    switch (columnKey) {
      ...
      case "nigeriaLocalRunning":
        return <p className="text-bold text-small">{cellValue}</p>;
      case "nigeriaOtherRunning":
        return <p className="text-bold text-small">{cellValue}</p>;
      
      case "overseeDta":
        return <p className="text-bold text-small">{cellValue}</p>;
      case "ovsLocalRunning":
        return <p className="text-bold text-small">{cellValue}</p>;
      case "ovsOtherRunning":
        return <p className="text-bold text-small">{cellValue}</p>;
      case "actions":
        return (
...
export default TrainingFeeTable;

TrainingFeeTable.propTypes = {
