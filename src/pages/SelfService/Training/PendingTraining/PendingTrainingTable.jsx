...
const PendingTrainingTable = ({handleOpenDrawer, rows}) => {
  ...
  const filteredItems = React.useMemo(() => {
    ...
    if (hasSearchFilter) {
      filteredrows = filteredrows.filter((user) =>
        user.creator.toLowerCase().includes(filterValue.toLowerCase())
      );
    }
    if (
      statusFilter !== "all" &&
  ...
  const renderCell = React.useCallback((user, columnKey) => {
    ...
    switch (columnKey) {
      case "_id":
        return (
          <p className="text-bold text-sm capitalize text-default-500">{cellValue}</p>
        );
      case "creator":
        return (
          <p className="text-bold text-sm capitalize text-default-500">{cellValue}</p>
      ...
      case "directorate":
        return (
          <div className="flex flex-col">
            <p className="text-sm text-default-500 max-w-[35vh]">{cellValue}</p>
          </div>
        );
      case "title":
        return <p className="text-bold text-sm capitalize text-default-500 w-[35vh]">{cellValue}</p>;
      case "description":
        return <p className="text-bold text-sm capitalize text-default-500 w-[35vh]">{cellValue}</p>;
      ...
      <TableHeader columns={headerColumns}>
        {(column) => (
          <TableColumn
            ...
            className="uppercase font-Exoti"
          >
            {column.name}
          </TableColumn>
        )}
...
export default PendingTrainingTable;

PendingTrainingTable.propTypes = {
