...
import { approvedTableHeader, approved_columns, status, statusColor, approvedTableData,} from "./data";
import { MdError, MdFilterList } from "react-icons/md";
import PropTypes from 'prop-types'

const ApprovedHistoryTable = ({setCurrentUser,view }) => {


  const [filterValue, setFilterValue] = React.useState("");
  const [selectedKeys, setSelectedKeys] = React.useState(new Set([]));
  const [visibleColumns, setVisibleColumns] = React.useState(
    new Set(approvedTableHeader)
  );
  ...
  const headerColumns = React.useMemo(() => {
    if (visibleColumns === "all") return approved_columns;

    return approved_columns.filter((column) =>
      Array.from(visibleColumns).includes(column.uid)
    );
  }, [visibleColumns]);

  const filteredItems = React.useMemo(() => {
    let filteredOnboardings = [...approvedTableData];
  ...
  }, [approvedTableData, filterValue, statusFilter]);
    ...
    return (
      <div className="flex flex-col gap-4">
        <Input
          ...
          placeholder="Filter by name, approved by and status"
        ...
        <div className="flex justify-between items-center">
          <span className="font-medium text-small">
            Total: {approvedTableData.length} onboardings
          </span>
...
ApprovedHistoryTable.propTypes={
