...
import { pendingTableHeader, pending_columns, status, statusColor, pendingTableData } from "./data";
import { MdFilterList } from "react-icons/md";
import PropTypes from 'prop-types'

const PendingHistoryTable = ({setCurrentUser,view }) => {


  const [filterValue, setFilterValue] = React.useState("");
  const [selectedKeys, setSelectedKeys] = React.useState(new Set([]));
  const [visibleColumns, setVisibleColumns] = React.useState(
    new Set(pendingTableHeader)
  );
  ...
  const headerColumns = React.useMemo(() => {
    if (visibleColumns === "all") return pending_columns;

    return pending_columns.filter((column) =>
      Array.from(visibleColumns).includes(column.uid)
    );
  }, [visibleColumns]);

  const filteredItems = React.useMemo(() => {
    let filteredOnboardings = [...pendingTableData];
  ...
  }, [pendingTableData, filterValue, statusFilter]);
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="flex justify-between items-center">
          <span className="font-medium text-small">
            Total: {pendingTableData.length} onboardings
          </span>
