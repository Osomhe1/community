...
const HistoryTable = ({ view, setCurrentViewExpenses,tableData }) => {
  ...
  const filteredItems = React.useMemo(() => {
    let filteredExpenses = [...tableData];
  ...
  const renderCell = React.useCallback((expense, columnKey) => {
    ...
    switch (columnKey) {
      ...
      case "approvals":
        return (
        <div>
        {expense.status=='pending'?<p className="text-center">-</p>: <AvatarGroup classNames={{
          base: "bg-[#f83f37] text-white font-[700] text-[10px]",
        }} size="sm" isBordered max={3}>
            {expense.approvals.map((_, i) => (
              <Avatar
                size="sm"
                key={i}
      ...
      case "actions":
        return (
          <div className="text-center">
             {/* {expense.status=='pending'?<Dropdown>
              <DropdownTrigger>
              ...
              <DropdownMenu>
                <DropdownItem onClick={() => viewExpenses(expense)}>View</DropdownItem>
                <DropdownItem>Approve</DropdownItem>
                <DropdownItem>Reject</DropdownItem>
              </DropdownMenu>
            </Dropdown>:<Button size='sm' className='bg-[#00BCC2] font-medium text-white px-2' onClick={() => viewExpenses(expense)} >View</Button>} */}
            <Button size='sm' className='bg-[#00BCC2] font-medium text-white px-2' onClick={() => viewExpenses(expense)} >View</Button>
          </div>
        );
      default:
        return cellValue;
    ...
    return (
      <div className="flex flex-col gap-4 font-medium">
        <Input
          ...
          variant="bordered"
          classNames={{
            inputWrapper: ["outline-none border-1 bg-white"]
          }}
        />
