...
const HistoryTable = ({ view, setCurrentViewLoan }) => {
  ...
  const filteredItems = React.useMemo(() => {
    let filteredLoans = [...tableData];

    if (hasSearchFilter) {
      filteredLoans = filteredLoans.filter(
        (loan) =>
          loan.type.toLowerCase().includes(filterValue.toLowerCase()) ||
          loan.amount
            .toLowerCase()
            .includes(filterValue.toLowerCase()) ||
          loan.interest_rate.toLowerCase().includes(filterValue.toLowerCase())
      );
    }
    if (
      statusFilter !== "all" &&
      ...
      filteredLoans = filteredLoans.filter((loan) =>
        Array.from(statusFilter).includes(loan.status)
      );
    }
  ...
  const renderCell = React.useCallback((loan, columnKey) => {
    ...
    return (
      <div className="flex flex-col gap-4 font-medium">
        <Input
          ...
          placeholder="Filter by type, interest rate and amount"
...
HistoryTable.propTypes = {
  setCurrentViewLoan: PropTypes.func,
  view: PropTypes.func,
};
