...
const HistoryTable = ({ view, setCurrentViewpayment }) => {
  ...
  const filteredItems = React.useMemo(() => {
    let filteredPayments = [...tableData];

    if (hasSearchFilter) {
      filteredPayments = filteredPayments.filter(
        (payment) =>
          payment.payment_type.toLowerCase().includes(filterValue.toLowerCase()) ||
          payment.provider
            .toLowerCase()
            .includes(filterValue.toLowerCase()) ||
          payment.paid_amount.toLowerCase().includes(filterValue.toLowerCase())
      );
    }
    if (
      statusFilter !== "all" &&
    ...
    return (
      <div className="flex flex-col gap-4 font-medium">
        <Input
          ...
          placeholder="Filter by type, provider and amount"
...
HistoryTable.propTypes = {
  setCurrentViewpayment: PropTypes.func,
  view: PropTypes.func,
};
